...
$periodo = trim($_POST['alunoPeriodo']);
...
if (isset($_POST['adicionar-aluno'])){
    ...
    if (empty($nome)){
        ...
        $sql->execute(array($nome,$cpf,$ra,$email,$celular,$periodo,$turno,$senha,$curso,$tipo));
...
// Verifica se o formulário de atualizar curso foi enviado
    ...
    if (empty($nome)){
        ...
        $sql = $pdo->prepare("UPDATE aluno SET nome_alu = ?, cpf_alu = ?, ra_alu = ?, email_alu = ?, celular_alu = ?, periodo_alu = ?, turno_alu = ?, status_alu = ?, senha_alu = ?, curso_idcur = ?, tp_u_idtpu = ? WHERE idalu = ?");
        $sql->execute(array($nome,$cpf,$ra,$email,$celular,$periodo,$turno,$status,$senha,$curso,$tipo,$alunoId));
